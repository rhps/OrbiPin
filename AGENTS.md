...
| # | Guidance | Rule in one line | OrbiPin needs it |
|---|---|---|---|
| 1 | Code Budget Ladder | 7 rungs, first "yes" wins; YAGNI is the kill switch | ✅ YES — core discipline for agent-built code |
| 2 | `ponytail:` debt markers | corner-cuts only with a stated ceiling; grep-stable; audit before milestones | ✅ YES — speed without silent landmines |
| 3 | Done means verified | typecheck, lint, gates run; report what ran | ✅ YES — kills "should work" reports |
| 4 | One thing at a time | no drive-by refactors, no smuggled features | ✅ YES |
| 5 | Spec authority | hackathon doc + `hackathon.md` are contracts; contradicting code fails review | ✅ YES |
| 6 | G1–G6 quality gates | precision tiers, boundary lookup, dedup, staleness, framing types, tone config | ✅ YES — the product's own law |
| 7 | `npm run gate` blocks deploy | one command, all six gates, red = no deploy | ✅ YES — the enforcement spine |
| 8 | Trunk-based main | `main` is the only integration branch; staging/prod are deploy targets, not branches | ✅ YES |
| 9 | PRs never deploy envs | PRs get CI + throwaway preview deploy only | ✅ YES |
| 10 | Tag-gated releases (release-please) | Release PR → tag vX.Y.Z → prod deploy; rollback anchors free | ✅ YES |
| 11 | Env-scoped secrets | runtime keys via `convex env set` per deployment; repo holds only deploy creds | ✅ YES |
| 12 | Sandbox quarantine | experiments/discussions/temp in `sandbox/`, never pushed | ✅ YES |
| 13 | Strict TypeScript everywhere | strict mode; the type system is the first guardrail | ✅ YES — G5 depends on it |
| 14 | Ingestion licensing rules | official interfaces (llms.txt/RSS) or permissive licenses only; never scrape what has an official API | ✅ YES — directly shapes the source whitelist |
| 15 | Write path ≠ read path | ingestion = cron batch writes; reads = cache-heavy, pre-aggregated | ✅ YES — crons ingest, live queries read |
| 16 | 300-line frontend file gate | CI lint blocks frontend files >300 lines | ⏳ LATER — from the first real frontend sprint |
| 17 | Framework stack defaults (Hono/D1/etc.) | Cloudflare-native defaults | OUT — OrbiPin is Convex-mandated (hackathon rule); deviation is the hackathon's law |
| 18 | JWT/refresh auth patterns | auth stack defaults | OUT — no auth planned (rules say optional) |
| 19 | SQL repository layer / parameterized SQL | SQL rules | OUT — Convex is not SQL-shaped; its own validators apply |
| 20 | ML feature-correctness / PIT / monitoring specs | ML systems law | OUT — no ML training pipeline in scope |
...
1. **Code Budget Ladder**: Does this need to exist?
